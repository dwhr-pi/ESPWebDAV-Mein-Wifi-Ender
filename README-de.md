...
Kopiere nun die Dateien von [FYSETC/ESPWebDAV](https://github.com/FYSETC/ESPWebDAV "") darauf. 
Anschlißend öffne und editiere die Datei `Setup.ini` im Verzeichnis `ini`. 
...
Schritt: Öffne das Programm `pronterface` 
...
Schritt: Jetzt kann man unten in der Befehlszeile die drei nachfolgenden notwendigen Befehle`M50 DeinWLAN-Netzwerknamen` als Befehl angeben. 
`M51 DeinWLAN-Passwort` 
`M52` 
Zurückgegeben wird nun im grauen Ausgabefenster Deine gegenwärtige IP-Adresse für das Gerät. 


`\\Deine-IP-Adresse\DavWWWRoot` ist das sehr einfache Webinterface Deines Druckers nun küftig erreichbar, es wird weil das Interface nicht entwickelt ist, anstelle einer Fehlerangabe des Browsers einen Fehlerbericht erhalten. 
```
Not found
URI: /DavWWWRoot Method: GET
```

Somit kann man die mSD-Karte von der SD-Karte nun als ein Netzwerk-Laufwerk mit in den Computer einbinden und dies sollte funktionieren. 
Bedenkt, das Netzwerklaufwerk steht nur dann zur Verfügung, wenn auch der Drucker eingeschaltet wurde. 
In Deinem Router sollte unter dieser IP-Adresse das Gerät `FYSETC` nun erscheinen, ändere den Namen oder passe die IP-Adresse als feste an. 
